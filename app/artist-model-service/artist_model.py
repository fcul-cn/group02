...
def connect():
    try:
        conn = psycopg2.connect(
            ...
            database=os.environ.get('POSTGRES_DB_ARTIST')
        )
        return conn
...
class ArtistService(app_pb2_grpc.ArtistService):
    def getArtist(self, request, context):
        try:
            ...
            conn.commit()
            if not (row is None):
                return GetArtistResponse(track=Artist(
                        artist_id=row[0],
                        artist_name=row[1],
                        artist_url=row[2],
    ...
    def addArtist(self, request, context):
        ...
        try:
            ...
            if cur.fetchone():
                raise AlreadyExists()
            # Add artist
            cur.execute(query, (artist_name, artist_url, request.artist_updated_at))
            artist_id = cur.fetchone()[0]
            conn.commit()
            return AddArtistResponse(artist_id=artist_id)
        except (psycopg2.DatabaseError) as error:
            print(error)
        finally:
            if conn is not None:
